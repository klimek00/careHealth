...
export default function SpecialtyAside({data: {specjalizacje}}) {
  /**
    ...
    <section className='flex flex-row w-1/4 mt-10 text-black '>
      <div className='z-10 h-auto w-36 flex flex-col bg-neutral-100 shadow shadow-neutral-400'>
        {specjalizacje.map((specialty, i) => (
          <div className='w-full py-2 px-4 text-sm hover:bg-violet-400 hover:text-white' key={i} title={specialty.opis}>{specialty.nazwa}</div>
        ))}
      </div>
